# train_FTM_SW.py
# Adapted from train_FTM_RB.py for ShallowWater VSR task.
#
# Key differences vs RB:
#   - 200 sequences (train 160), 72 frames per sequence
#   - HR 128x128, R = (1, 128, 128) same as RB
...
def load_sw_train_data(device):
    """Load HR training sequences directly from npz files, return normalized tensor."""
    print("Loading HR training data from npz...")
    hr_list = []
    for i in range(N_TRAIN):
        hr = np.load(f'{HR_DIR}/sample_{i:03d}.npz')['output'].astype(np.float32)  # (72, 128, 128)
        hr_list.append(hr)
    hr_data = np.stack(hr_list)  # [160, 72, 128, 128]
    ...
    # Add D=1 dim: [160, 72, 1, 128, 128]
    hr_norm_5d = hr_norm[:, :, np.newaxis, :, :]

    data_tensor = torch.FloatTensor(hr_norm_5d)  # keep on CPU, move batch to GPU in training loop
...
def train_parallel(basis_function, tucker_core, loader, optimizer, max_iter,
    ...
    for iter in tqdm(range(max_iter)):
        ...
        if iter > 800 and (loss < getattr(train_parallel, '_loss_min', 10) or iter % 50 == 0):
            ...
            if rmse < RMSE_min:
                RMSE_min = rmse
                core_path  = f"./data/core_sw_{R[0]}x{R[1]}x{R[2]}_{current_time}.mat"
                basis_path = f"./ckp/basis_sw_{R[0]}x{R[1]}x{R[2]}_{current_time}.pth"
                stats_path = f"./data/norm_stats_sw_{current_time}.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch_size",    type=int,   default=16)  # smaller batch to fit GPU memory
    config = parser.parse_args()

    # Load data directly from npz
    data, data_min, data_max = load_sw_train_data(device)
    # data: [160, 72, 1, 128, 128] on CPU
