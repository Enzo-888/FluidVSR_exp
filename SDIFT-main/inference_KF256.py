# inference_KF256.py
# Adapted from inference_RB.py for KF256 (Kolmogorov Flow) VSR task.
#
# Key differences vs RB:
#   - HR 256x256, LR 64x64 (RB: 128x128, 32x32)
#   - 180 frames per sequence (RB: 100)
#   - 70 sequences, test split: indices 63-69 (7 seqs)
#   - File pattern: trajectory_*.npz
#   - sample_shape uses [1, 180, 1, 256, 256]
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 python inference_KF256.py \
#     --basis_path ./ckp/basis_kf256_1x256x256_XXXX.pth \
#     --model_path ./exps/gp-edm_kf256_XXXX/checkpoints/ema_XXXXX.pth \
#     --core_mean_std_path ./exps/gp-edm_kf256_XXXX/core_mean_std.mat \
#     --norm_stats_path ./data/norm_stats_kf256_XXXX.json \
...
def build_lr_observations(lr_seq_norm, t_ind_uni):
    ...
    Args:
        lr_seq_norm: [T, 1, 64, 64] normalized LR for one test sequence
    ...
    # LR coordinates: u=1.0 (D=1 single point), v and w on linspace(0,1,64)
    u_val  = np.array([1.0], dtype=np.float32)
    lr_v   = np.linspace(0, 1, 64, dtype=np.float32)
    lr_w   = np.linspace(0, 1, 64, dtype=np.float32)
    vv, ww = np.meshgrid(lr_v, lr_w, indexing='ij')
    uu     = np.ones_like(vv) * u_val[0]
    lr_coords = np.stack([uu.ravel(), vv.ravel(), ww.ravel()], axis=1)  # [4096, 3]
    ...
    for t in range(T):
        y_t = lr_seq_norm[t, 0, :, :].ravel().astype(np.float64)  # [4096]
        y_group.append(y_t)
...
def compute_continuous_poest(x_0, basis_function, core_mean, core_std, core_t,
    ...
    x_0_vec = x_0.view(core_tensor_shape[0], core_tensor_shape[1], -1)
    ...
    for y, y_tt, y_t_ind, ind in zip(y_group, y_time_group, y_time_ind_group, ind_conti_group):
        ...
        x_0_t    = x_0_denorm[0, y_t_ind, :]
        y_tensor = torch.DoubleTensor(y).to(device)
        ind_tensor = torch.FloatTensor(ind).to(device)
        A = basis_function(input_ind_sampl=ind_tensor).detach().double()
        ...
        x_0_remove    = x_0_denorm[:, t_remove_group, :]
        ...
        post          = A.T @ (y_tensor - A @ x_0_aggregate)
...
def decoder(u_ind_uni, v_ind_uni, w_ind_uni, core, basis_function, target_size=None):
    """
    core: [T, 1, R_h, R_w] tensor (on device)
    Returns numpy array [T, 1, target_size, target_size] (or [T, 1, R_h, R_w] if target_size is None)
    """
    ...
    if target_size is not None and output.shape[-1] != target_size:
        output = F.interpolate(output, size=(target_size, target_size), mode='bicubic', align_corners=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--norm_stats_path',    type=str,   required=True,
                        help='norm_stats_kf256_*.json saved by train_FTM_KF256.py')
    parser.add_argument('--output_dir',         type=str,   default='./output_kf256')
    # DPS hyperparameters
    ...
    # ---- Load norm stats (saved by train_FTM_KF256.py) ----
    ...
    # ---- Fixed coordinates (match FTM R dimension, not HR resolution) ----
    u_ind_uni = np.array([1.0], dtype=np.float32)
    hr_v_ind  = np.linspace(0, 1, 64, dtype=np.float32)   # R=64, decode at R resolution
    hr_w_ind  = np.linspace(0, 1, 64, dtype=np.float32)
    t_ind_uni = np.linspace(0, 1, 180, dtype=np.float32)

    # ---- Load test data directly from npz ----
    HR_DIR = '/data/yc/dataset/KF256_data/x4/HR'
    LR_DIR = '/data/yc/dataset/KF256_data/x4/LR'
    # 70 seqs: train 56 (0-55), valid 7 (56-62), test 7 (63-69)
    N_TRAIN, N_VAL = 56, 7
    test_indices = list(range(N_TRAIN + N_VAL, 70))  # 63-69

    print(f"Loading {len(test_indices)} test sequences from npz...")
    hr_raw_list, lr_raw_list = [], []
    for i in test_indices:
        hr_raw_list.append(np.load(f'{HR_DIR}/trajectory_{i:03d}.npz')['output'].astype(np.float32))
        lr_raw_list.append(np.load(f'{LR_DIR}/trajectory_{i:03d}.npz')['output'].astype(np.float32))

    hr_raw = np.stack(hr_raw_list)  # [7, 180, 256, 256]
    lr_raw = np.stack(lr_raw_list)  # [7, 180, 64, 64]

    # Normalize
    hr_norm = (hr_raw[:, :, np.newaxis, :, :] - data_min) / (data_max - data_min)  # [7, 180, 1, 256, 256]
    lr_norm = (lr_raw[:, :, np.newaxis, :, :] - data_min) / (data_max - data_min)  # [7, 180, 1, 64, 64]
    ...
    for i in range(N_test):
        ...
        # Tucker decode: [T, 1, 64, 64] → bicubic upsample → [T, 1, 256, 256], normalized [0,1]
        basis_function.mode = "sampling"
        out_norm = decoder(u_ind_uni, hr_v_ind, hr_w_ind, core_sample[0], basis_function, target_size=256)

        # Denormalize to physical values
        out_phys = out_norm * (data_max - data_min) + data_min

        # Reformat to (T, H, W, 1)
        pred_phys = out_phys.transpose(0, 2, 3, 1)   # [T, 256, 256, 1]
        gt_phys   = hr_raw[i][:, :, :, np.newaxis]   # [T, 256, 256, 1]
        lr_phys   = lr_raw[i][:, :, :, np.newaxis]   # [T, 64, 64, 1]
    ...
    # ---- Save results ----
    pred_all = np.concatenate(pred_list, axis=0).astype(np.float32)  # [1260, 256, 256, 1]
    gt_all   = np.concatenate(gt_list,   axis=0).astype(np.float32)
    lr_all   = np.concatenate(lr_list,   axis=0).astype(np.float32)  # [1260, 64, 64, 1]
