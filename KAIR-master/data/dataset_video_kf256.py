"""KF256 VSR dataset for VRT/RVRT (KAIR framework).

Loads HR/LR pairs from .npz files.  Returns {'L': lq, 'H': gt, 'key': ...}
following the KAIR convention used by model_vrt.py.

Each .npz file: 'output' array of shape (180, H, W) for HR or (180, h, w) for LR.
HR: 256x256, LR: 64x64, scale: 4.
"""
...
class VideoRecurrentKF256Dataset(Dataset):
    """KF256 VSR dataset compatible with the KAIR/VRT training loop.

    Args:
        opt (dict): Config dict with keys:
            dataroot_gt (str): HR .npz directory.
            dataroot_lq (str): LR .npz directory.
            num_samples (int): Number of sequences to load. Default: 70.
            num_frame (int | None): Frames per training clip. None -> full seq.
            train_ratio (float): Fraction for training. Default: 0.8.
            valid_ratio (float): Fraction for validation. Default: 0.1.
            split (str): 'train', 'valid', or 'test'. Default: 'train'.
            use_mirror_sequence (bool): Temporal mirror for training. Default: True.
            test_mode (bool): If True, disable augmentation. Default: False.
    """
    ...
    def __init__(self, opt):
        ...
        num_samples = opt.get('num_samples', 70)
        ...
        self.use_mirror_sequence = opt.get('use_mirror_sequence', True) and not self.test_mode
        ...
        stats_path = osp.join(
            ...
            f'KF256_{num_samples}_norm_stats.json')
        ...
        if split == 'train':
            ...
            stats = {
                'hr_min': float(all_hr.min()), 'hr_max': float(all_hr.max()),
                'lr_min': float(all_lr.min()), 'lr_max': float(all_lr.max()),
            }
            with open(stats_path, 'w') as f:
                json.dump(stats, f, indent=2)
            print(f'[KF256 Dataset] Norm stats saved to {stats_path}')
        else:
            assert osp.exists(stats_path), (
                f'Norm stats not found at {stats_path}. '
                f'Build the train dataset first.')
        ...
        self.hr_seqs = [(seq - self.hr_min) / hr_range for seq in self.hr_seqs]
        self.lr_seqs = [(seq - self.lr_min) / lr_range for seq in self.lr_seqs]

        # ── Build index ───────────────────────────────────────────────────────
        self.data_infos = []
        T = self.num_frame if self.num_frame is not None else self.frames_per_seq
    ...
    def __getitem__(self, idx):
        ...
        return {'L': lq_t, 'H': gt_t, 'key': key, 'folder': key}

    # ── Denorm helpers (used by main_test_vrt_kf256.py) ──────────────────────
