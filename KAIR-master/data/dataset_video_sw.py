"""SW VSR dataset for VRT/RVRT (KAIR framework).

Loads HR/LR pairs from .npz files.  Returns {'L': lq, 'H': gt, 'key': ...}
following the KAIR convention used by model_vrt.py.

Each .npz file: 'output' array of shape (72, H, W) for HR or (72, h, w) for LR.
HR: 128x128, LR: 32x32, scale: 4.
"""
...
class VideoRecurrentSWDataset(Dataset):
    """SW VSR dataset compatible with the KAIR/VRT training loop.
    ...
    def __init__(self, opt):
        ...
        num_samples = opt.get('num_samples', 200)
        ...
        stats_path = osp.join(
            ...
            f'SW_{num_samples}_norm_stats.json')
        ...
        if split == 'train':
            ...
            print(f'[SW Dataset] Norm stats saved to {stats_path}')
        else:
    ...
    # ── Denorm helpers (used by main_test_vrt_sw.py) ──────────────────────
