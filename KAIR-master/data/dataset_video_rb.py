"""RayleighBenard VSR dataset for VRT/RVRT (KAIR framework).

Loads HR/LR pairs from .npz files.  Returns {'L': lq, 'H': gt, 'key': ...}
following the KAIR convention used by model_vrt.py.

Each .npz file: 'output' array of shape (100, H, W) for HR or (100, h, w) for LR.
...
class VideoRecurrentRBDataset(Dataset):
    """RayleighBenard VSR dataset compatible with the KAIR/VRT training loop.

    Args:
        opt (dict): Config dict with keys:
            dataroot_gt (str): HR .npz directory.
            dataroot_lq (str): LR .npz directory.
            num_samples (int): Number of sequences to load. Default: 120.
    ...
    def __init__(self, opt):
        ...
        self.num_frame = opt.get('num_frame', None)   # None → full sequence
    ...
    # ── Denorm helpers (used by main_test_vrt_rb.py) ─────────────────────────
