...
class EfficiencyTracker:
    ...
    def save_stats(self, checkpoint_path=None, model_size_mb=None, params_total=None):
        ...
        # ── 自动同步到中央 efficiency_stats 目录 ──
        self._sync_to_central(stats)

        return output_file

    def _sync_to_central(self, stats):
        """将效率 JSON 合并写入 /data/yc/Fluid_VSR/efficiency_stats/"""
        CENTRAL_DIR = Path('/data/yc/Fluid_VSR/efficiency_stats')
        DATASET_MAP = {
            'RB': 'RB', 'KF256': 'KF256',
            'SW': 'ShallowWater', 'ShallowWater': 'ShallowWater',
            'ERA5V': 'ERA5V',
            'KF256_train': 'KF256', 'RB_train': 'RB',
            'SW_train': 'ShallowWater', 'ERA5V_train': 'ERA5V',
        }
        try:
            CENTRAL_DIR.mkdir(parents=True, exist_ok=True)
            std_name = DATASET_MAP.get(self.dataset_name, self.dataset_name)
            central_path = CENTRAL_DIR / f'{self.model_name}_{std_name}.json'

            if central_path.exists():
                with open(central_path, 'r') as f:
                    existing = json.load(f)
                for key in stats.get('training', {}):
                    if stats['training'][key] is not None:
                        existing.setdefault('training', {})[key] = stats['training'][key]
                for key in stats.get('inference', {}):
                    if stats['inference'][key] is not None:
                        existing.setdefault('inference', {})[key] = stats['inference'][key]
                for key in ['params_total', 'params_trainable', 'model_size_mb']:
                    if stats.get(key) is not None:
                        existing[key] = stats[key]
                existing['dataset'] = std_name
                merged = existing
            else:
                merged = dict(stats)
                merged['dataset'] = std_name

            with open(central_path, 'w') as f:
                json.dump(merged, f, indent=2)
            print(f"  Synced to: {central_path}")
        except Exception as e:
            print(f"  Warning: failed to sync to central stats: {e}")
