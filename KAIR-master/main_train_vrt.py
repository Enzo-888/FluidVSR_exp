...
def main(json_path='options/vrt/001_train_vrt_videosr_bi_reds_6frames.json'):
    ...
    if opt['rank'] == 0:
        ...
        tracker.start_training(batch_size=batch_size, device=device)
        logger.info(f"Efficiency tracking started: batch_size={batch_size}, device={device}")
    else:
        tracker = None
    ...
    for epoch in range(1000000):  # keep running
        for i, train_data in enumerate(train_loader):
            ...
            if current_step > opt['train']['total_iter']:
                ...
                if opt['rank'] == 0 and tracker is not None:
                    tracker.end_training(total_iters=current_step)
                    checkpoint_path = os.path.join(opt['path']['models'], f'{current_step}_G.pth')
                    tracker.save_stats(checkpoint_path=checkpoint_path)
