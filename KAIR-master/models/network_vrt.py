...
class VRT(nn.Module):
    ...
    def get_flow_2frames(self, x):
        ...
        # SpyNet expects 3-channel input; replicate if single-channel
        if c != 3:
            x_1 = x_1.repeat(1, 3, 1, 1)
            x_2 = x_2.repeat(1, 3, 1, 1)

        # SpyNet's 5-level pyramid requires input >= 64px;
        # upsample temporarily and rescale flows back if input is smaller.
        ...
        def _rescale_flows(flows):
            """Interpolate flows from SpyNet spatial size back to LR size and scale displacements."""
            ...
            for i, flow in enumerate(flows):
                ...
                flow[:, 0] *= w / spy_w   # x-displacement
                flow[:, 1] *= h / spy_h   # y-displacement
                rescaled.append(flow)
            return rescaled

        # backward
        flows_backward = _rescale_flows(self.spynet(x_1, x_2))
        flows_backward = [flow.view(b, n-1, 2, h // (2 ** i), w // (2 ** i)) for flow, i in
                          zip(flows_backward, range(4))]

        # forward
        flows_forward = _rescale_flows(self.spynet(x_2, x_1))
