...
class BaseForecaster(object):
    ...
    def vis_video_sequence(self, seq_x, seq_y, normalizer, frame_indices,
                           save_path=None, dpi=100, **kwargs):
        """
        Visualize a video sequence as a 3-row grid.

        Args:
            seq_x: (T, h, w, 1)  LR frames of one sequence (from test_dataset)
            seq_y: (T, H, W, 1)  HR frames of one sequence
            normalizer: used for decoding predictions and ground truth
            frame_indices: list of int, the frame indices to visualize (e.g. [0,24,49,74,99])
            save_path: optional path to save the figure
        Layout:
            Row 0: Ground Truth (5 frames)
            Row 1: Prediction   (5 frames)   <- shared colorbar with Row 0
            Row 2: Absolute Error (5 frames) <- own colorbar
        """
        n = len(frame_indices)
        x = seq_x[frame_indices].to(self.device)   # (n, h, w, 1)
        y = seq_y[frame_indices].to(self.device)    # (n, H, W, 1)

        self.model.eval()
        with torch.no_grad():
            y_pred = self.inference(x, y, **kwargs)  # (n, H, W, 1)

        y_pred = normalizer.decode(y_pred).cpu()
        y      = normalizer.decode(y).cpu()
        abs_err = torch.abs(y_pred - y)

        # squeeze channel dim -> (n, H, W)
        gt_np   = y.squeeze(-1).numpy()
        pred_np = y_pred.squeeze(-1).numpy()
        err_np  = abs_err.squeeze(-1).numpy()

        vmin, vmax   = gt_np.min(), gt_np.max()
        err_vmax     = err_np.max()

        fig, axs = plt.subplots(3, n, figsize=(n * 3, 10), dpi=dpi,
                                constrained_layout=True)

        titles = [f't={i}' for i in frame_indices]
        row_labels = ['Ground Truth', 'Prediction', 'Abs Error']

        for col in range(n):
            im_gt   = axs[0, col].imshow(gt_np[col],   cmap='viridis', vmin=vmin,  vmax=vmax)
            im_pred = axs[1, col].imshow(pred_np[col],  cmap='viridis', vmin=vmin,  vmax=vmax)
            im_err  = axs[2, col].imshow(err_np[col],   cmap='hot',     vmin=0,     vmax=err_vmax)
        ...
        # shared colorbar for GT and Prediction rows
        fig.colorbar(im_gt,   ax=axs[0:2, :], location='right', shrink=0.6, pad=0.01)
        # separate colorbar for error row
        fig.colorbar(im_err,  ax=axs[2,   :], location='right', shrink=0.6, pad=0.01)
