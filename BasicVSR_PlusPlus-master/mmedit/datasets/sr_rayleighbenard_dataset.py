...
class SRRayleighBenardDataset(Dataset):
    """RayleighBenard VSR dataset for BasicVSR++.

    Loads HR/LR pairs directly from .npz files (no PNG conversion needed).
    Handles train/val/test splits by sequence index, computes and caches
    per-split normalization statistics (min-max to [0, 1]).

    Each .npz file contains one simulation sequence:
        'output': (100, H, W)  — 100 frames at HR (128x128) or LR (32x32)

    Args:
        hr_path (str): Directory containing HR sample_*.npz files.
        lr_path (str): Directory containing LR sample_*.npz files.
        pipeline (list[dict]): Pipeline transforms. For this dataset, typically
            just [dict(type='Collect', keys=['lq', 'gt'], ...)].
        scale (int): SR upscale factor. Default: 4.
        split (str): One of 'train', 'valid', 'test'. Default: 'train'.
        num_samples (int): Number of sequences to load. Default: 100.
        num_input_frames (int | None): Frames per training clip. If None, the
            full sequence (100 frames) is returned. Default: None.
        train_ratio (float): Fraction of sequences for training. Default: 0.8.
        valid_ratio (float): Fraction of sequences for validation. Default: 0.1.
        use_mirror_sequence (bool): If True, double the training clip by
            temporally mirroring it (same trick as BasicVSR++ paper).
            Default: True.
    ...
    def __init__(self,
        ...
        self.frames_per_seq = self.hr_seqs[0].shape[0]  # typically 100
        ...
        if split == 'train':
            # Compute from training sequences and save
            ...
            print(f'[RB Dataset] Norm stats saved to {stats_path}')
        else:
        ...
        # ── Build data_infos ──────────────────────────────────────────────────
        # For test/valid: one entry per full sequence
        # For train: one entry per valid window start position
        self.data_infos = []
        ...
        # Build pipeline (typically just Collect)
    ...
    def __getitem__(self, idx):
        ...
        if not self.test_mode:
            ...
            if np.random.random() < 0.5:                    # vertical flip
                lq = lq[:, ::-1, :].copy()
                gt = gt[:, ::-1, :].copy()
            if np.random.random() < 0.5:                    # HW transpose
        ...
        key = f'seq{seq_idx + 1:04d}'
        results = dict(
            lq=lq_t,
            gt=gt_t,
            lq_path=osp.join(self.lr_path, f'seq{seq_idx:04d}'),
            gt_path=osp.join(self.hr_path, f'seq{seq_idx:04d}'),
            key=key,
            scale=self.scale,
        )
    ...
    def evaluate(self, results, logger=None):
        """Placeholder — proper evaluation done via eval_rb.ipynb."""
        return {}

    # ── Helpers for denormalisation (used by test_rb.py) ─────────────────────

    def denorm_hr(self, x):
        """Denormalise HR tensor/array from [0,1] to physical values."""
        return x * (self.hr_max - self.hr_min) + self.hr_min

    def denorm_lr(self, x):
        """Denormalise LR tensor/array from [0,1] to physical values."""
