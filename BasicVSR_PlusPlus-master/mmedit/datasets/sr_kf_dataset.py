...
class SRKFDataset(Dataset):
    """Kolmogorov Flow (KF256) VSR dataset for BasicVSR++.

    Loads HR/LR pairs from trajectory_*.npz files.
    Each .npz contains:
        'output': (T, H, W) — T frames at HR (256×256) or LR (64×64)

    Args:
        hr_path (str): Directory containing HR trajectory_*.npz files.
        lr_path (str): Directory containing LR trajectory_*.npz files.
        ...
        num_samples (int): Number of trajectories to load. Default: 70.
    ...
    def __getitem__(self, idx):
        ...
        results = dict(
            lq=lq_t,
            gt=gt_t,
            lq_path=osp.join(self.lr_path, f'trajectory_{seq_idx:03d}.npz'),
            gt_path=osp.join(self.hr_path, f'trajectory_{seq_idx:03d}.npz'),
            key=key,
            scale=self.scale,
        )
