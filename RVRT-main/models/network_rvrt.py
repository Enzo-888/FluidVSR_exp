...
class RVRT(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_last = nn.Conv3d(64, in_chans, kernel_size=(1, 3, 3), padding=(0, 1, 1))
    ...
    def compute_flow(self, lqs):
        ...
        # SpyNet expects 3-channel RGB; replicate if single-channel
        if c != 3:
            lqs_1 = lqs_1.repeat(1, 3, 1, 1)
            lqs_2 = lqs_2.repeat(1, 3, 1, 1)

        # SpyNet's 5-level pyramid requires input >= 64px
        spy_h, spy_w = max(h, 64), max(w, 64)
        if spy_h != h or spy_w != w:
            lqs_1 = F.interpolate(lqs_1, size=(spy_h, spy_w), mode='bilinear', align_corners=False)
            lqs_2 = F.interpolate(lqs_2, size=(spy_h, spy_w), mode='bilinear', align_corners=False)
        ...
        flows_backward = _rescale(self.spynet(lqs_1, lqs_2)).view(n, t - 1, 2, h, w)

        if self.is_mirror_extended:  # flows_forward = flows_backward.flip(1)
            flows_forward = None
        else:
            flows_forward = _rescale(self.spynet(lqs_2, lqs_1)).view(n, t - 1, 2, h, w)
